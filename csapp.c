 ...
 */
/* $begin rio_readn */
ssize_t rio_readn(int fd, void *usrbuf, size_t n) // 식별자 fd의 현재 파일 위치에서 메모리 위치 usrbuf로 최대 n 바이트 전송한다.
{
    size_t nleft = n;
    ssize_t nread;
    char *bufp = usrbuf;
    ...
    while (nleft > 0)
    {
        if ((nread = read(fd, bufp, nleft)) < 0) // 시스템 콜 수준에서의 에러
        {
    ...
    return (n - nleft); /* Return >= 0 */ /* 요청한 바이트 중 얼마나 읽어들였는지 */
}
/* $end rio_readn */
